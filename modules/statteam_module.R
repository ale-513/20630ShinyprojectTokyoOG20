...
statteamServer <- function(id, data) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    # 2. Observe team selection -> update matches
    
    
    # 2. Observe team selection -> update matches
    observeEvent(input$team, {
      req(input$team)
      matches <- unique(data$file_name[data$team == input$team])
    ...
    filtered_data <- reactive({
      req(input$team, input$skill, input$matches)
      data %>%
    ...
    heatmap_serve <- reactive({
      if (input$skill == "Serve") {
        filtered_data() %>% 
          filter(skill == "Serve",team == input$team) %>% 
          mutate(evaluation = case_when(
    ...
    heatmap_reception <- reactive({
      if (input$skill == "Reception") {
        ...
        filtered_data() %>% 
          filter(skill == "Reception",team == input$team) %>% 
          mutate(evaluation = case_when(
    ...
    heatmap_dig <- reactive({
      if (input$skill == "Dig") {
        ...
        filtered_data() %>%
          filter(skill == "Dig", team == input$team) %>% 
          mutate(evaluation = case_when(
    ...
    output$attackPlot <- renderPlot({
      if (input$plot_type_attack == "Zone Heatmap" && !is.null(zone_attack())) {
        ggplot(zone_attack(), aes(x, y, fill = .data[[input$evaluation_zone_attack]])) +
          ...
          ggcourt(labels = input$team, court = "upper") +
        ...
        ggplot(hx_attack, aes(x, y, fill = density)) +
          ...
          labs(
            fill = "Density",
            title = paste(input$team, input$evaluation_density_attack, "density") 
          )
        ...
        ggplot(heatmap_attack() %>% filter(evaluation %in% input$evaluation_trajectory_attack), 
          ...
          ggcourt(labels = paste(input$team, "Attacking")) +
          labs(title = paste("Attack Trajectories", input$team))
      }
    })
    ...
    output$servePlot <- renderPlot({
      if (input$plot_type_serve == "Zone Heatmap" && !is.null(zone_serve())) {
        ...
        ggplot(hx_serve, aes(x, y, fill = density)) +
          ...
          labs(
            fill = "Density",
            title = paste(input$team, input$evaluation_density_serve, "density")
          )
        ...
        ggplot(heatmap_serve() %>% filter(evaluation %in% input$evaluation_trajectory_serve),
          ...
          scale_colour_manual(values = c(
            ...
            "Other" = "gray50"),
            name = "Evaluation") +
          ggcourt(labels = paste(input$team, "Serving")) + 
          labs(title = paste("Serve Trajectories", input$team)) 
      }
    })
    ...
    output$receptionPlot <- renderPlot({
      if (input$plot_type_reception == "Zone Heatmap" && !is.null(zone_reception())) {
        ...
        ggplot(hx_reception, aes(x, y, fill = density)) +
          ...
          labs(
            fill = "Density",
            title = paste(input$team, input$evaluation_density_reception, "density")
          )
      }
    })
    ...
    output$blockPlot <- renderPlot({
      if (input$plot_type_block == "Zone Heatmap" && !is.null(zone_block())) {
        ggplot(zone_block(), aes(x, y, fill = .data[[input$evaluation_zone_block]])) +
          ...
          ggcourt(labels = input$team, court = "lower") + 
        ...
        ggplot(hx_block, aes(x, y, fill = density)) +
          ...
          labs(
            fill = "Density",
            title = paste(input$team, input$evaluation_density_block, "density") 
          )
      }
    })
    outputOptions(output, "blockPlot", suspendWhenHidden = FALSE)
    ...
    output$digPlot <- renderPlot({
      if (input$plot_type_dig == "Zone Heatmap" && !is.null(zone_dig())) {
        ...
        ggplot(hx_dig, aes(x, y, fill = density)) +
          ...
          labs(
            fill = "Density",
            title = paste(input$team, input$evaluation_density_dig, "density") 
          )
      }
    })
    outputOptions(output, "digPlot", suspendWhenHidden = FALSE)
