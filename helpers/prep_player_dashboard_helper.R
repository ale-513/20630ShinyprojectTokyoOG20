...
preprocess_data <- function(data) {
  df <- data %>% 
    ...
    mutate(across(-c(player_name, team), as.numeric))
  
  # Create individual dataframes as list elements
  list(
    best_scorer = df %>% select(player_name, team, total_points, WL, attack_points, ace_serve, block_points),
    best_attacker = df %>% select(player_name, team, total_attack, attack_points, attack_errors, attack_blocked, attack_point_percentage, attack_efficiency),
    best_blocker = df %>% select(player_name, team, block_points, block_errors, block_positive, block_poor, block_efficiency),
    best_server = df %>% select(player_name, team, total_serve, ace_serve, error_serve, bad_serve, positive_serve, serve_point_percentage, serve_efficiency),
    best_digger = df %>% select(player_name, team, successfull_dig, perfect_dig, good_dig, error_dig, freeball_dig, dig_efficiency),
    best_receiver = df %>% select(player_name, team, total_reception, perfect_reception, positive_reception, bad_reception, error_reception, reception_positivity, reception_efficiency)
  )
}


preprocess_match_data <- function(data) {
  data %>%
    group_by(team, player_name, file_name) %>%
    summarize(
